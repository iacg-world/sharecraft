...
onMounted(() => {
  canvasContainer = document.querySelector('.canvas-container') as HTMLElement
})
    ...
    'top-left': () => {
      ...
      if (store && store.state.editor.gridSettings.enabled) {
        ...
        if (canvasContainer) {
          ...
          result.top = Math.round(relativeTop / spacing) * spacing + canvasTop
        }
      }

      // 宽度限制：确保组件宽度不超过画布宽度
      result.width = parseInt(
        limitComponentWidth(result.width + 'px', canvasWidth).replace('px', ''),
      )
